...
class class_wxthird
{
    ...
    public function __construct()
    {
        ...
        if (isset($_SERVER['HTTP_APPNAME'])){        //SAE环境，需要开通memcache
            $mem = memcache_init();
            $this->component_verify_ticket = $mem->get("component_verify_ticket");
            $this->component_access_token = $mem->get("component_access_token");
            if (!isset($this->component_access_token) || empty($this->component_access_token)){
                ...
                $mem->set("component_access_token", $this->component_access_token, 0, 7200);
            }
        }else{
    ...
    private function logger($log_content)
    {
        ...
        }else if($_SERVER['REMOTE_ADDR'] != "127.0.0.122"){ //LOCAL
            $max_size = 500000;
            $log_filename = "class.xml";
