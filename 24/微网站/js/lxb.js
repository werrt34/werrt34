...
function(a) {
    var c = lxb.use("base");
    var e = {
        BDCBID: '6dc2a8bb-b3b8-4184-bce1-cdff8f555b2b',
        LXBVT: 0,
        TEMPSITEID: '3049852',
...
function(d) {
    var f = {};
    var i = preHeight = preTop = preLeft = preTipHeight = 0;
    var j = {};
    var a = function(m, l) {
        for (var k in m) {
            if ((m.hasOwnProperty && m.hasOwnProperty(k)) || (!m.hasOwnProperty)) {
                if (! (k in ["event"])) {
                    l[k] = m[k]
                }
            }
    ...
    d.show = function() {
        j.tipEle.style.display = "";
        c();
        if (preTipHeight != f.tip.height || i != f.con.width || preHeight != f.con.height || preTop != f.con.top || preLeft != f.con.left) {
            d.resetLoc();
            i = f.con.width;
    ...
    d.resetLoc = function() {
        var k = (f.con.height <= f.tip.height) ? true: false;
        if (f.tip.width > f.con.left) {
            if (f.tip.height > f.con.top) {
                j.arrow.className = "arrow-left-t";
                j.arrow.style.top = "6px";
                j.tipEle.style.top = k ? "22px": "10px"
            } else {
                j.arrow.className = "arrow-left-b";
                j.tipEle.style.top = k ? (f.con.height - f.tip.height - 10) + "px": "0px"
            }
            j.tipEle.style.left = (f.con.left + f.con.width + 7 + j.value) + "px"
        } else {
            if (f.tip.height > f.con.top) {
                j.arrow.className = "arrow-right-t";
                j.arrow.style.top = "6px";
                j.tipEle.style.top = k ? "22px": "10px"
            } else {
                j.arrow.className = "arrow-right-b";
                j.tipEle.style.top = k ? (f.con.height - f.tip.height - 10) + "px": "0px"
            }
            j.tipEle.style.left = ( - f.tip.width - 7 - 10) + "px"
        }
    };
    d.hide = function() {
        j.tipEle.style.display = "none"
    }
});
...
function(e) {
    ...
    function b() {
        var t = h.MAIN;
        var o = k.main = c.create("ins", {
            id: i.MAIN
        });
        o.style.visibility = "hidden";
        if (f.floatColor) {
            o.style.backgroundColor = f.floatColor
        }
        if (f.imagePath) {
            o.style.backgroundImage = 'url("' + f.imagePath + '")'
        }
        var u = h.MAIN + "-" + f.style + "-" + f.type;
        var m = 0;
        var q = 0;
        if (f.position > 0) {
            m = 1
        ...
        q = Math.abs(f.position);
        var s = h.Position.HOR[m];
        u += " " + u + h.PositionFix.HOR[m];
        o.className = t + " " + u + " " + s + " " + r;
        if (q <= 45) {
            o.className += " " + h.Position.VER[0]
        } else {
            o.className += " " + h.Position.VER[1]
        }
        var n = k.btnHide = c.create("ins", {
            className: h.MAIN + "-btn-hide"
        });
        o.appendChild(n);
        n = k.btnShow = c.create("ins");
        t = h.MAIN + "-btn-show";
        t += " " + h.MAIN + "-btn-show-" + f.style;
        t += " " + s;
        t += " " + h.MAIN + "-btn-show-" + f.style + h.PositionFix.HOR[m];
        n.id = i.SHOW;
        n.className = t;
        ...
        if (! (g.isStandard() && g.isHorizon())) {
            ...
            if (f.floatColor) {
                n.style.backgroundColor = f.floatColor
            }
            if (f.imagePath) {
                n.style.backgroundImage = 'url("' + f.imagePath + '")'
            }
            n.appendChild(btnBg)
        }
        var p = function(w) {
            var v = g.css(o, "zIndex");
            if (v > 0) {
                w.call(null)
            } else {
                setTimeout(function() {
        ...
        p(function() {
            ...
            var v = c.viewportSize();
            q = (q == 1 ? 0 : q);
            k.btnShow.style.top = o.style.top = (q / 100 * (v.height - j)) + "px";
            a()
        });
        if (c.ie && c.ie <= 6) {
            ...
            o.appendChild(k.mask);
            n.appendChild(k.btnShowMask = c.create("iframe", {
                frameBorder: 0,
                className: h.MAIN + "-mask"
            }))
        }
...
function(a) {
    ...
    a.init = function(e) {
        ...
        var g = e.btnShow;
        var f = b.MAIN + "-custom";
        f += " " + b.MAIN + "-custom-" + e.type + "-" + (e.windowLayout == 1 ? "h": "v");
        f += " " + b.MAIN + "-custom-" + (e.windowLayout == 1 ? "h": "v") + (Math.abs(e.position) == 50 ? b.PositionFix.VER[1] : "");
        c.className += " " + f;
        var d = e.url;
        if (d.indexOf("?") >= 0) {
            d += "&"
